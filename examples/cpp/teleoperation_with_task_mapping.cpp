...
Eigen::Matrix<double, 4, 4> T_right_ma = Eigen::Matrix<double, 4, 4>::Identity();
Eigen::Matrix<double, 4, 4> T_left_ma = Eigen::Matrix<double, 4, 4>::Identity();
...
std::pair<Eigen::Matrix<double, 4, 4>, Eigen::Matrix<double, 4, 4>> ComputeFK(std::shared_ptr<rb::dyn::Robot<14>> robot,
                                                                              std::shared_ptr<State<14>> state,
                                                                              Eigen::Matrix<double, 14, 1> q_joint) {

  Eigen::Matrix<double, 4, 4> T_right, T_left;

  state->SetQ(q_joint);
  robot->ComputeForwardKinematics(state);

  T_right = robot->ComputeTransformation(state, 0, 1);
  T_left = robot->ComputeTransformation(state, 0, 2);

  return std::make_pair(T_right, T_left);  // / unit [Nm]
}
...
void control_loop_for_master_arm(dynamixel::PortHandler* portHandler, dynamixel::PacketHandler* packetHandler,
  ...
  while (true) {
    ...
    auto rv_fk = ComputeFK(robot, state, q_joint);
...

    {
      ...
      q_joint_ma = q_joint;
      T_right_ma = rv_fk.first;
      T_left_ma = rv_fk.second;
    }
...
int main(int argc, char** argv) {
...

  {
    ...
    Eigen::Matrix<double, 4, 4> T_torso_fixed, T_torso_ref;
    T_torso_fixed.setIdentity();
    T_torso_ref.setIdentity();

    Eigen::Matrix<double, 4, 4> T_right_ma_fixed, T_left_ma_fixed;
    T_right_ma_fixed.setIdentity();
    T_left_ma_fixed.setIdentity();
    ...
    T_torso_ref = dyn->ComputeTransformation(dyn_state, 0, 1);
...

      {
        std::lock_guard<std::mutex> lg(mtx_q_joint_ma_info);
        {
          ...
          if (hand_controller_button(0)) {
            ...
            T_torso_ref = T_torso_fixed * (T_right_ma_fixed.inverse() * T_right_ma);
            T_torso_ref(0, 3) = std::clamp(T_torso_ref(0, 3), -0.15, 0.15);
            T_torso_ref(1, 3) = std::clamp(T_torso_ref(1, 3), -0.15, 0.15);
            T_torso_ref(2, 3) = std::clamp(T_torso_ref(2, 3), 0.7, 1.3);
          } else {
            right_arm_minimum_time = 1.0;

            T_right_ma_fixed = T_right_ma;

            T_torso_fixed = dyn->ComputeTransformation(dyn_state, 0, 1);
          }
      ...
      command_builder.SetCommand(ComponentBasedCommandBuilder().SetBodyCommand(
          BodyComponentBasedCommandBuilder()
              ...
              .SetTorsoCommand(CartesianCommandBuilder()
                                   .AddTarget("base", "link_torso_5", T_torso_ref, 1, 3.141592, 3)
                                   .SetMinimumTime(right_arm_minimum_time)
